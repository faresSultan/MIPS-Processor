...
wire [31:0]WriteData;  // **
wire [4:0]Write_register;  // number of the register to be written in RegDst dependant
wire [31:0] Read_data2,Read_data1; // outputs of RF
wire RegWrite; // RF Write enable
...
PC PC(.clk(clk),.rst_n(rst_n),.nextInstruction(nextInstructionAddress),.currentInstruction(currentInstructionAddress));

       // Fetching the current insturction from the Memory
    assign Instruction = {InstructionMemory[currentInstructionAddress],InstructionMemory[currentInstructionAddress+1],
    InstructionMemory[currentInstructionAddress+2],InstructionMemory[currentInstructionAddress+3]};
   
    assign nextInstructionAddress = currentInstructionAddress+4;  // modify when implement branch
//=====================================


//----Decode Instruction

RegisterFile RF (.Read_data1(Read_data1),.Read_data2(Read_data2),.Write_data(WriteData),
.Write_register(Write_register),.Read_register1(Instruction[25:21]),.Read_register2(Instruction[20:16]),.RegWrite(RegWrite));

ControlUnit CU (.RegDst(RegDst),.Branch(Branch),.MemRead(MemRead),.MemtoReg(MemtoReg),.ALUOp(ALUOp),
.MemWrite(MemWrite),.ALUSrc(ALUSrc),.RegWrite(RegWrite),.opCode(opCode));

Sign_extend SE (.in(Instruction[15:0]),.out(ImmiedaiteValue));

ALU_control ALUctr(.ALUOp(ALUOp),.instruction(Instruction[5:0]),.ALUfunction(ALUfunction));



    assign Write_register = (RegDst == 0)? Instruction[20:16] : Instruction[15:11];
